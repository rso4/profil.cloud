...
@section('content')
{{-- Navbar --}}
<nav class="bg-emerald-700 sticky top-0 z-50 shadow-md">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex space-x-6 text-emerald-50">
            ...
            <a href="#tahfidz" class="hover:text-white">Tahfidz</a>
            <a href="#kontak" class="hover:text-white">Kontak</a>
            @include('templates.partials.nav-pages')
        </div>
        <a href="#pendaftaran" class="bg-white text-emerald-700 px-6 py-2.5 rounded-lg font-semibold">Pendaftaran</a>
    </div>
</nav>
...
<section id="beranda" class="relative text-white">
    <img src="{{
                $profile->cover_image_url ?: static_image('school-03', 'hero', 'islamic school with mosque dome, students in uniform, peaceful green campus', 'landscape_16_9')
    ...
    <div class="relative max-w-7xl mx-auto px-4 py-48 text-center">
        <p class="text-emerald-100 text-2xl mb-6">بِسْمِ اللهِ الرَّحْمَنِ الرَّحِيْمِ</p>
        <h1 class="text-4xl md:text-6xl font-extrabold mb-6">{{ $tenant->name }}</h1>
        <p class="text-xl text-emerald-50 mb-10 max-w-2xl mx-auto">{{ $profile->tagline ?? 'Mendidik generasi berakhlak mulia' }}</p>
        <a href="#tentang" class="bg-white text-emerald-700 px-8 py-3 rounded-lg font-bold">Kenali Kami</a>
    </div>
</section>
...
<section id="tentang" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        <img src="{{
                $profile->about_image_url ?: static_image('school-03', 'full', 'students reading quran together in classroom', 'square')
        ...
        <div>
            ...
            <h2 class="text-3xl md:text-4xl font-bold mb-6 text-gray-900">Pendidikan Berbasis Iman dan Taqwa</h2>
            ...
            <div class="grid grid-cols-3 gap-6">
                <div class="text-center">
                    ...
                    <div class="text-sm text-gray-500">Hari Tahfidz</div>
                </div>
                <div class="text-center">
                ...
                <div class="text-center">
                    ...
                    <div class="text-sm text-gray-500">Hafidz</div>
                </div>
            </div>
        </div>
    </div>
...
<section id="program" class="py-20 bg-emerald-50">
    <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl md:text-4xl font-bold text-center mb-12 text-gray-900">Program Pendidikan</h2>
        <div class="grid md:grid-cols-3 gap-8">
            @forelse($services as $service)
                <div class="bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition">
                    ...
                    @if($service->price)
                        <span class="text-emerald-600 font-bold text-lg">Rp {{ number_format($service->price, 0, ',', '.') }}</span>
                    @endif
                </div>
            @empty
...
{{-- Tahfidz --}}
<section id="tahfidz" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 text-center">
        <h2 class="text-3xl md:text-4xl font-bold mb-6 text-gray-900">Program Tahfidz Al-Quran</h2>
        <p class="text-gray-600 max-w-3xl mx-auto mb-8">Program khusus menghafal Al-Quran dengan bimbingan guru hafidz profesional.</p>
        ...
        <div class="grid md:grid-cols-3 gap-8 text-center">
            @forelse($contacts as $contact)
                <div class="p-6">
                    ...
                    <div class="text-emerald-100 text-sm mb-1">{{ $contact->label }}</div>
                    <div class="font-medium text-lg">{{ $contact->value }}</div>
                </div>
            @empty
                <p class="col-span-3 text-emerald-100">Belum ada kontak.</p>
            @endforelse
        </div>
    </div>
</section>
...
<section id="pendaftaran" class="py-16 bg-white">
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <p class="text-gray-600 mb-8">Daftarkan putra-putri Anda untuk menjadi generasi qurani.</p>
        <a href="#kontak" class="bg-emerald-600 text-white px-8 py-3 rounded-lg font-bold">Info Pendaftaran</a>
    </div>
</section>

<footer class="bg-emerald-900 text-emerald-100 py-6 text-center">
