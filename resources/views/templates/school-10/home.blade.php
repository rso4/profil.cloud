...
@section('content')
{{-- Navbar --}}
<nav class="bg-cyan-700 sticky top-0 z-50 shadow-md">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <a href="/" class="text-2xl font-bold text-white">{{ $tenant->name }}</a>
        <div class="hidden md:flex space-x-6 text-cyan-50">
        ...
        <a href="#pendaftaran" class="bg-white text-cyan-700 px-6 py-2.5 rounded-full font-bold">Daftar</a>
    </div>
</nav>
...
<section id="beranda" class="relative text-white">
    ...
    <div class="relative max-w-7xl mx-auto px-4 py-48 text-center">
        <p class="uppercase tracking-[0.3em] text-sm mb-4">Pendidikan Inklusif</p>
        <h1 class="text-4xl md:text-6xl font-extrabold mb-6">{{ $tenant->name }}</h1>
        <p class="text-xl text-cyan-50 mb-10 max-w-2xl mx-auto">{{ $profile->tagline ?? 'Setiap anak berhak belajar dan berprestasi' }}</p>
        <a href="#tentang" class="bg-white text-cyan-700 px-8 py-3 rounded-full font-bold">Kenali Kami</a>
    </div>
</section>
...
<section id="tentang" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
            ...
            <h2 class="text-3xl md:text-4xl font-bold mb-6 text-gray-900">Kami Peduli Setiap Anak</h2>
            ...
            <div class="grid grid-cols-3 gap-6">
                <div class="text-center">
                    ...
                    <div class="text-sm text-gray-500">Inklusif</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-cyan-600">{{ $services->count() }}</div>
                    <div class="text-sm text-gray-500">Layanan</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-cyan-600">20</div>
...
{{-- Layanan --}}
<section id="layanan" class="py-20 bg-cyan-50">
        ...
        <div class="grid md:grid-cols-3 gap-8">
            @forelse($services as $service)
                <div class="bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition">
                    ...
                    @if($service->price)
                        <span class="text-cyan-600 font-bold text-lg">Rp {{ number_format($service->price, 0, ',', '.') }}</span>
                    @endif
                </div>
            @empty
...
{{-- Kontak --}}
<section id="kontak" class="py-20 bg-cyan-700 text-white">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid md:grid-cols-3 gap-8 text-center">
            @forelse($contacts as $contact)
                <div class="p-6">
                    ...
                    <div class="text-cyan-100 text-sm mb-1">{{ $contact->label }}</div>
                    <div class="font-medium text-lg">{{ $contact->value }}</div>
                </div>
            @empty
                <p class="col-span-3 text-cyan-100">Belum ada kontak.</p>
            @endforelse
        </div>
    </div>
</section>
...
<section id="pendaftaran" class="py-16 bg-white">
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <p class="text-gray-600 mb-8">Setiap anak istimewa, dan kami siap mendampingi.</p>
        <a href="#kontak" class="bg-cyan-600 text-white px-8 py-3 rounded-full font-bold">Info Pendaftaran</a>
    </div>
</section>

<footer class="bg-cyan-900 text-cyan-100 py-6 text-center">
