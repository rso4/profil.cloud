...
<nav class="bg-blue-700 sticky top-0 z-50 shadow-md">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex space-x-6 text-blue-50">
        ...
        <a href="#pendaftaran" class="bg-white text-blue-700 px-6 py-2.5 rounded-lg font-semibold">Pendaftaran</a>
    </div>
</nav>
...
<section id="beranda" class="relative text-white">
    <img src="{{
                $profile?->cover_image_url ?: static_image('school-01', 'hero', 'modern school building with students in uniform smiling, bright campus', 'landscape_16_9')
    ...
    <div class="relative max-w-7xl mx-auto px-4 py-44 text-center">
        <p class="uppercase tracking-[0.3em] text-sm text-blue-100 mb-4">Selamat Datang di</p>
        <h1 class="text-4xl md:text-6xl font-extrabold mb-6">{{ $tenant->name }}</h1>
        <p class="text-xl text-blue-50 mb-10 max-w-2xl mx-auto">{{ $profile->tagline ?? 'Menumbuhkan generasi cerdas dan berkarakter' }}</p>
        <a href="#pendaftaran" class="bg-white text-blue-700 px-8 py-3 rounded-lg font-bold">Daftar Sekarang</a>
    </div>
</section>
...
<section id="tentang" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        <div>
            <p class="text-blue-600 font-semibold tracking-wide mb-2">Tentang Kami</p>
            <h2 class="text-3xl md:text-4xl font-bold mb-6 text-gray-900">Membangun Masa Depan Cerah</h2>
            <p class="text-gray-600 leading-relaxed mb-6">{{ $profile->description ?? 'Deskripsi sekolah belum diisi.' }}</p>
            <div class="flex gap-8">
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">{{ $services->count() }}</div>
                    <div class="text-sm text-gray-500">Program</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">100%</div>
                    <div class="text-sm text-gray-500">Kelulusan</div>
                </div>
            </div>
        </div>
        <img src="{{
                $profile?->about_image_url ?: static_image('school-01', 'full', 'happy students in classroom learning together', 'square')
            }}" alt="Kegiatan Belajar" class="rounded-2xl h-80 w-full object-cover shadow-lg">
    </div>
</section>

{{-- Program --}}
<section id="program" class="py-20 bg-blue-50">
...
<section id="fasilitas" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl md:text-4xl font-bold text-center mb-12 text-gray-900">Fasilitas Sekolah</h2>
...
{{-- Galeri --}}
<section id="galeri" class="py-20 bg-blue-50">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid md:grid-cols-3 gap-8 text-center">
            @forelse($contacts as $contact)
                <div class="p-6">
                    ...
                    <div class="text-blue-100 text-sm mb-1">{{ $contact->label }}</div>
                    <div class="font-medium text-lg">{{ $contact->value }}</div>
                </div>
            @empty
                <p class="col-span-3 text-blue-100">Belum ada kontak.</p>
            @endforelse
        </div>
    </div>
</section>
...
<section id="pendaftaran" class="py-16 bg-white">
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <p class="text-gray-600 mb-8">Segera daftarkan putra-putri Anda untuk tahun ajaran baru.</p>
        <a href="#kontak" class="bg-blue-600 text-white px-8 py-3 rounded-lg font-bold">Info Pendaftaran</a>
    </div>
</section>
