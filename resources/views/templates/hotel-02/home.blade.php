@extends('layouts.base')

@section('content')
{{-- Navbar --}}
<nav class="bg-white/90 backdrop-blur sticky top-0 z-50 border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <a href="/" class="text-2xl font-serif font-bold text-primary">{{ $tenant->name }}</a>
        <div class="hidden md:flex space-x-8 text-sm tracking-wide text-gray-600">
            <a href="#beranda" class="hover:text-primary">Beranda</a>
            <a href="#tentang" class="hover:text-primary">Tentang</a>
            <a href="#kamar" class="hover:text-primary">Kamar</a>
            <a href="#fasilitas" class="hover:text-primary">Fasilitas</a>
            <a href="#galeri" class="hover:text-primary">Galeri</a>
            <a href="#kontak" class="hover:text-primary">Kontak</a>
            @include('templates.partials.nav-pages')
        </div>
        <a href="#kontak" class="btn-primary text-white px-6 py-2.5 rounded-full text-sm font-medium">Reservasi</a>
    </div>
</div>

{{-- Hero --}}
<section id="beranda" class="relative text-white">
    <img src="{{
                $profile->cover_image_url ?: static_image('hotel-02', 'hero', 'luxury hotel lobby with elegant interior, warm lighting, marble floors, grand chandelier', 'landscape_16_9')
            }}" alt="Hotel {{ $tenant->name }}" class="absolute inset-0 w-full h-full object-cover">
    <div class="absolute inset-0 bg-black/50"></div>
    <div class="relative max-w-7xl mx-auto px-4 py-40 text-center">
        <p class="uppercase tracking-[0.3em] text-sm text-gray-200 mb-4">Selamat Datang di</p>
        <h1 class="text-4xl md:text-6xl font-serif font-bold mb-6">{{ $tenant->name }}</h1>
        <p class="text-xl text-gray-200 mb-10 max-w-2xl mx-auto">{{ $profile->tagline ?? 'Pengalaman menginap yang berkelas' }}</p>
        <a href="#kamar" class="btn-primary text-white px-8 py-3 rounded-full font-medium">Lihat Kamar & Harga</a>
    </div>
</section>

{{-- Tentang --}}
<section id="tentang" class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        <div>
            <p class="text-primary font-semibold tracking-wide mb-2">Tentang Kami</p>
            <h2 class="text-3xl md:text-4xl font-serif font-bold mb-6 text-gray-900">Penginapan Mewah untuk Pengalaman Tak Terlupakan</h2>
            <p class="text-gray-600 leading-relaxed mb-6">{{ $profile->description ?? 'Deskripsi hotel belum diisi.' }}</p>
            <div class="flex gap-6">
                <div class="text-center">
                    <div class="text-3xl font-bold text-primary">{{ $services->count() }}+</div>
                    <div class="text-sm text-gray-500">Layanan</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-primary">{{ $galleries->count() }}+</div>
                    <div class="text-sm text-gray-500">Galeri</div>
                </div>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <img src="{{
                $profile->about_image_url ?: static_image('hotel-02', 'full', 'hotel bedroom with luxurious bedding, soft lighting, elegant decor', 'square')
            }}" alt="Kamar" class="rounded-2xl h-48 w-full object-cover shadow-lg">
            <img src="{{
                static_image('hotel-02', 'full', 'hotel restaurant fine dining, elegant table setting', 'square')
            }}" alt="Restoran" class="rounded-2xl h-48 w-full object-cover shadow-lg mt-8">
        </div>
    </div>
</section>

{{-- Kamar --}}
<section id="kamar" class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4">
        <p class="text-primary text-center tracking-wide uppercase text-sm font-semibold mb-2">Kamar & Tarif</p>
        <h2 class="text-3xl md:text-4xl font-serif font-bold text-center mb-12 text-gray-900">Tipe Kamar Kami</h2>
        <div class="grid md:grid-cols-3 gap-8">
            @forelse($services as $service)
                <div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition">
                    <div class="h-40 bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-5xl">
                        {{ $service->icon ?? '🛏️' }}
                    </div>
                    <div class="p-6">
                        <h3 class="text-xl font-semibold mb-2 text-gray-900">{{ $service->name }}</h3>
                        <p class="text-gray-600 text-sm mb-4">{{ $service->description }}</p>
                        @if($service->price)
                            <div class="flex items-baseline gap-1">
                                <span class="text-primary font-bold text-2xl">Rp {{ number_format($service->price, 0, ',', '.') }}</span>
                                <span class="text-gray-500 text-sm">/malam</span>
                            </div>
                        @endif
                    </div>
                </div>
            @empty
                <p class="col-span-3 text-center text-gray-500">Belum ada kamar.</p>
            @endforelse
        </div>
    </div>
</section>

{{-- Fasilitas --}}
<section id="fasilitas" class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl md:text-4xl font-serif font-bold text-center mb-12 text-gray-900">Fasilitas Unggulan</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
            @forelse($contacts->take(4) as $contact)
                <div class="text-center p-6 rounded-2xl bg-gray-50 hover:bg-primary hover:text-white transition">
                    <div class="text-4xl mb-3">
                        @if($contact->type === 'phone') 📞 @elseif($contact->type === 'email') ✉️ @elseif($contact->type === 'address') 📍 @else 🌐 @endif
                    </div>
                    <div class="font-medium">{{ $contact->label }}</div>
                    <div class="text-sm text-gray-500">{{ $contact->value }}</div>
                </div>
            @empty
                <p class="col-span-4 text-center text-gray-500">Belum ada fasilitas.</p>
            @endforelse
        </div>
    </div>
</section>

{{-- Galeri --}}
<section id="galeri" class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl md:text-4xl font-serif font-bold text-center mb-12 text-gray-900">Galeri</h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @forelse($galleries as $gallery)
                <img src="{{ $gallery->thumb_url }}" alt="{{ $gallery->title }}" data-gallery data-fullscreen="{{ $gallery->image_url }}" class="w-full h-52 object-cover rounded-2xl shadow-sm hover:scale-105 transition duration-300">
            @empty
                <p class="col-span-4 text-center text-gray-500">Belum ada galeri.</p>
            @endforelse
        </div>
    </div>
</section>

{{-- Kontak --}}
<section id="kontak" class="py-20 bg-secondary text-white">
    <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl md:text-4xl font-serif font-bold text-center mb-12">Hubungi Kami</h2>
        <div class="grid md:grid-cols-3 gap-8 text-center">
            @forelse($contacts as $contact)
                <div class="p-6">
                    <div class="text-3xl mb-3">
                        @if($contact->type === 'phone') 📞 @elseif($contact->type === 'email') ✉️ @elseif($contact->type === 'address') 📍 @else 🌐 @endif
                    </div>
                    <div class="text-gray-400 text-sm mb-1">{{ $contact->label }}</div>
                    <div class="font-medium text-lg">{{ $contact->value }}</div>
                </div>
            @empty
                <p class="col-span-3 text-gray-400">Belum ada kontak.</p>
            @endforelse
        </div>
    </div>
</section>

<footer class="bg-gray-900 text-gray-400 py-8 text-center">
    <p>&copy; {{ date('Y') }} {{ $tenant->name }}. Powered by Profil Cloud.</p>
</footer>
@endsection
